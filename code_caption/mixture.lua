local Mixture, parent = torch.class('nn.Mixture', 'nn.Module')

function Mixture:__init(dim)
   parent.__init(self)
   self.dim = dim
...
function Mixture:updateOutput(input) 
...
function Mixture:updateGradInput(input, gradOutput)
   local gaterInput, expertInputs = table.unpack(input)
...
function Mixture:type(type, tensorCache)
   self._gaterView = nil
   self._expert = nil
